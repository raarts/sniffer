...
void *handle_skinny2(pcap_pkthdr *header, const u_char *packet, unsigned int saddr, int source, unsigned int daddr, int dest, char *data, int datalen, int dataoffset,
	...
	case SET_RINGER_MESSAGE:
		SKINNY_DEBUG(DEBUG_PACKET, 3, "Received SET_RINGER_MESSAGE from %s\n", "d->name");
		break;
	case SET_SPEAKER_MESSAGE:
		SKINNY_DEBUG(DEBUG_PACKET, 3, "Received SET_SPEAKER_MESSAGE from %s\n", "d->name");
		break;
	case SET_LAMP_MESSAGE:
		SKINNY_DEBUG(DEBUG_PACKET, 3, "Received SET_LAMP_MESSAGE from %s\n", "d->name");
		break;
	case CALL_STATE_MESSAGE:
		{
		unsigned int ref = letohl(req.data.callstate.callReference);
		int state = letohl(req.data.callstate.callState);
		SKINNY_DEBUG(DEBUG_PACKET, 3, "Received CALL_STATE_MESSAGE state [%u] ref [%u] \n", state, ref);
		...
		case SKINNY_RINGOUT:
			...
			call->lastSIPresponseNum = 183;
			break;
		case SKINNY_RINGIN:
			strcpy(call->lastSIPresponse, "RING IN");
			call->lastSIPresponseNum = 183;
			break;
		case SKINNY_CONNECTED:
			...
			break;
		case SKINNY_BUSY:
			call->lastSIPresponseNum = 486;
			strcpy(call->lastSIPresponse, "BUSY");
			break;
		case SKINNY_CONGESTION:
			call->lastSIPresponseNum = 503;
			strcpy(call->lastSIPresponse, "CONGESTION");
			break;
		case SKINNY_HOLD:
			strcpy(call->lastSIPresponse, "HOLD");
			break;
		case SKINNY_CALLWAIT:
			strcpy(call->lastSIPresponse, "CALLWAIT");
			break;
		case SKINNY_TRANSFER:
			strcpy(call->lastSIPresponse, "TRANSFER");
			break;
		case SKINNY_PARK:
			strcpy(call->lastSIPresponse, "PARK");
			break;
		case SKINNY_PROGRESS:
			call->lastSIPresponseNum = 100;
			strcpy(call->lastSIPresponse, "PROGRESS");
			break;
		case SKINNY_CALLREMOTEMULTILINE:
			strcpy(call->lastSIPresponse, "CALL REMOTE MULTILINE");
			break;
		case SKINNY_INVALID:
			call->lastSIPresponseNum = 603;
	...
	case START_TONE_MESSAGE:
		{
		unsigned int ref = letohl(req.data.starttone.reference);
		char callid[16];
	...
	case STOP_TONE_MESSAGE:
		{
		unsigned int ref = letohl(req.data.stoptone.reference);
		char callid[16];
	...
	case CALL_INFO_MESSAGE:
		{
		unsigned int ref = letohl(req.data.callinfo.reference);
		char callid[16];
	...
	case CM5CALL_INFO_MESSAGE:
		{
		unsigned int ref = letohl(req.data.cm5callinfo.reference);
		char callid[16];
...
/*
		...
		uint32_t space[36];
*/
		unsigned int ref = letohl(req.data.openreceivechannel.conferenceId);
		int pid = letohl(req.data.openreceivechannel.partyId);
		char callid[16];
	...
	case DIALED_NUMBER_MESSAGE:
		{
		unsigned int ref = letohl(req.data.dialednumber.callReference);
		char callid[16];
	...
	case DEFINETIMEDATE_MESSAGE:
		SKINNY_DEBUG(DEBUG_PACKET, 3, "Received DEFINETIMEDATE_MESSAGE from %s\n", "d->name");
		break;
	...
	case STOP_MEDIA_TRANSMISSION_MESSAGE:
		SKINNY_DEBUG(DEBUG_PACKET, 3, "Received STOP_MEDIA_TRANSMISSION_MESSAGE from %s\n", "d->name");
		break;
	case CLEAR_PROMPT_MESSAGE:
		SKINNY_DEBUG(DEBUG_PACKET, 3, "Received CLEAR_PROMPT_MESSAGE from %s\n", "d->name");
		break;
	case KEEP_ALIVE_MESSAGE:
		SKINNY_DEBUG(DEBUG_PACKET, 3, "Received KEEP_ALIVE_MESSAGE from %s\n", "d->name");
		break;
	case KEEP_ALIVE_ACK_MESSAGE:
		SKINNY_DEBUG(DEBUG_PACKET, 3, "Received KEEP_ALIVE_ACK_MESSAGE from %s\n", "d->name");
		break;
	case SELECT_SOFT_KEYS_MESSAGE:
		{
		SKINNY_DEBUG(DEBUG_PACKET, 3, "Received SELECT_SOFT_KEYS_MESSAGE instance [%u] state [%u] ref [%u] \n", req.data.selectsoftkey.instance, letohl(req.data.selectsoftkey.reference), letohl(req.data.selectsoftkey.softKeySetIndex));
	...
	case DISPLAY_PROMPT_STATUS_MESSAGE:
		{
		unsigned int ref = letohl(req.data.displaypromptstatus.callReference);
		char callid[16];
	...
	case ACTIVATE_CALL_PLANE_MESSAGE:
		SKINNY_DEBUG(DEBUG_PACKET, 3, "Received ACTIVATE_CALL_PLANE_MESSAGE from %s\n", "d->name");
		break;
...
*/
		SKINNY_DEBUG(DEBUG_PACKET, 3, "Received REGISTER_MESSAGE from %s\n", "d->name");
	case IP_PORT_MESSAGE:
		SKINNY_DEBUG(DEBUG_PACKET, 3, "Received IP_PORT_MESSAGE from %s\n", "d->name");
		//res = handle_ip_port_message(req, s);
		break;
	case KEYPAD_BUTTON_MESSAGE:
		SKINNY_DEBUG(DEBUG_PACKET, 3, "Received KEYPAD_BUTTON_MESSAGE from %s\n", "d->name");
#if 0
	    {
...
//		transmit_selectsoftkeys(d, 0, 0, KEYDEF_ONHOOK);
	...
	case SOFT_KEY_EVENT_MESSAGE:
		SKINNY_DEBUG(DEBUG_PACKET, 3, "Received SOFT_KEY_EVENT_MESSAGE ref %x\n", req.data.softkeyeventmessage.callreference);
...
static void *skinny_newcall(void *data)
{
	...
	/* XXX This code is probably not necessary */
