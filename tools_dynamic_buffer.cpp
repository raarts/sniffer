...
void ChunkBuffer::add(char *data, u_int32_t datalen, bool flush, u_int32_t decompress_len, bool directAdd) {
	...
	case add_simple: {
		...
		this->len += datalen;
		ATOMIC_FETCH_AND_ADD(&this->chunk_buffer_size, datalen);
		ATOMIC_FETCH_AND_ADD(&ChunkBuffer::chunk_buffers_sumsize, datalen);
		}
		break;
	case add_fill_chunks: {
		...
		this->len += allcopied;
		ATOMIC_FETCH_AND_ADD(&this->chunk_buffer_size, allcopied);
		ATOMIC_FETCH_AND_ADD(&ChunkBuffer::chunk_buffers_sumsize, allcopied);
		}
		break;
	case add_fill_fix_len: {
		...
		do {
			...
			ATOMIC_FETCH_AND_ADD(&this->chunk_buffer_size, whattocopy);
			ATOMIC_FETCH_AND_ADD(&ChunkBuffer::chunk_buffers_sumsize, whattocopy);
